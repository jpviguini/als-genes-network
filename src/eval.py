...
def plot_auc_comparison(results_list, metric_name, filename):
    ...
    plt.tight_layout()
    plt.savefig(f"{DATA_DIR}_{model_name}_{filename}.png", dpi=300)
    plt.close()
...
def plot_top_genes_heatmap(df_eval, top_n=50):
    ...
    plt.tight_layout()
    #plt.savefig(f"{DATA_DIR}heatmap_pageranked_top_{top_n}_novel_genes_BERT_{UMBRELLA_TERM}.png", dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    if all_metrics:
        ...
        res_df.to_csv(f"{DATA_DIR}metrics_L1_holdout_{UMBRELLA_TERM}.csv", index=False)
    else:
